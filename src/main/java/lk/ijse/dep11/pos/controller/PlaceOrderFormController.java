...
public class PlaceOrderFormController {
    ...
    public void initialize(){
        ...
        txtQty.textProperty().addListener((ov, prev, cur)->{
            ...
            btnSave.setDisable(!(cur.matches("\\d+") && Integer.parseInt(cur) <= selectedItem.getQty() && Integer.parseInt(cur) > 0));
        });
    ...
    public void btnAdd_OnAction(ActionEvent actionEvent) {
        ...
        if (optOrderItem.isEmpty()) {
            ...
            OrderItem newOrderItem = new OrderItem(selectedItem.getCode(), selectedItem.getDescription(), Integer.parseInt(txtQty.getText()), selectedItem.getUnitPrice(), btnDelete);
    ...
    public void calculateOrderTotal(){
        Optional<BigDecimal> orderTotal = tblOrderDetails.getItems().stream().map(oi -> oi.getTotal()).reduce(BigDecimal::add);
    ...
    public void btnPlaceOrder_OnAction(ActionEvent actionEvent) {
        try {
            OrderDataAccess.saveOrder(lblId.getText().replace("Order ID: ", "").strip(), Date.valueOf(lblDate.getText()), cmbCustomerId.getValue().getId(),
